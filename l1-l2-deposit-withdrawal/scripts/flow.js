...
const { predeploys, getContractInterface, getContractFactory } = require('@nahmii/contracts')
const relayer = require('@nahmii/message-relayer')

// Set up some contract factories. This can be ignored.
...
const factory__L1StandardBridge = getContractFactory('NVM_L1StandardBridge')
const factory__L2StandardBridge = getContractFactory('NVM_L2StandardBridge')
...
async function main() {
  ...
  const l1AddressManager = new ethers.Contract(
    //'0x357eCd03C1601273094Db3474A13D2452cF6785A', // Testnet address manager
    '0x7934915C03eA2E2C4D69c269F45598B738ddee08', // Mainnet address manager
  ...
  console.log(await relayer.getWithdrawalsByTxHashes([tx3.hash], watcher, l1AddressManager))

  console.log(`Waiting for withdrawal to be relayed to L1...`)
  const [ msgHash2 ] = await watcher.getMessageHashesFromL2Tx(tx3.hash)

  await relayer.relayXDomainMessages(tx3.hash, l1Messenger.address, l1RpcProvider, l2RpcProvider, l1Wallet)
